import { useState } from 'react';
import { useNavigate } from 'react-router-dom';
import { useProviders } from '../../../hooks/use-providers';
import { providerColumns } from './provider-columns';
import { ProviderCard } from './provider-card';
import { PageHeader } from '../../layout/PageHeader';
import { EmptyState } from '../../feedback/EmptyState';
import { LoadingPage } from '../../feedback/LoadingSpinner';
import { ErrorState } from '../../feedback/ErrorState';
import { Button } from '../../ui/button';
import { Input } from '../../ui/input';
import { Badge } from '../../common/Badge';
import { Building2, Plus, Search, LayoutGrid, List } from 'lucide-react';

export function ProviderList({ onCreateClick }) {
  const navigate = useNavigate();
  const [search, setSearch] = useState('');
  const [viewMode, setViewMode] = useState('list');

  const filters = {};
  if (search) filters.search = search;

  const { data: providers, isLoading, error, refetch } = useProviders(filters);

  const handleRowClick = (provider) => {
    navigate(`/providers/${provider.id}`);
  };

  if (isLoading) return <LoadingPage label="Loading providers..." />;
  if (error) return <ErrorState error={error} onRetry={refetch} />;

  const list = providers || [];

  return (
    <div className="flex flex-col gap-6">
      <PageHeader
        title="Providers"
        description="Manage your AI providers"
        icon={Building2}
        actions={
          <Button onClick={onCreateClick}>
            <Plus className="mr-2 h-4 w-4" />
            Add Provider
          </Button>
        }
      />

      <div className="flex items-center gap-3">
        <div className="relative flex-1 max-w-sm">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 size-4 text-muted-foreground" />
          <Input
            placeholder="Search providers..."
            value={search}
            onChange={(e) => setSearch(e.target.value)}
            className="pl-9"
          />
        </div>
        <div className="flex items-center gap-1 border rounded-md p-0.5">
          <Button
            variant={viewMode === 'list' ? 'secondary' : 'ghost'}
            size="icon"
            className="size-8"
            onClick={() => setViewMode('list')}
          >
            <List className="size-4" />
          </Button>
          <Button
            variant={viewMode === 'grid' ? 'secondary' : 'ghost'}
            size="icon"
            className="size-8"
            onClick={() => setViewMode('grid')}
          >
            <LayoutGrid className="size-4" />
          </Button>
        </div>
        <Badge variant="secondary">{list.length}</Badge>
      </div>

      {list.length === 0 ? (
        <EmptyState
          icon={Building2}
          title="No providers found"
          description={search ? 'Try a different search term.' : 'Add your first provider to get started.'}
          action={
            !search && (
              <Button onClick={onCreateClick}>
                <Plus className="mr-2 h-4 w-4" />
                Add Provider
              </Button>
            )
          }
        />
      ) : viewMode === 'grid' ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {list.map((provider) => (
            <ProviderCard
              key={provider.id}
              provider={provider}
              onClick={handleRowClick}
            />
          ))}
        </div>
      ) : (
        <div className="rounded-lg border">
          <table className="w-full">
            <thead>
              <tr className="border-b bg-muted/50">
                {providerColumns.map((col) => (
                  <th
                    key={col.accessorKey || col.id}
                    className="h-10 px-4 text-left text-xs font-medium text-muted-foreground"
                  >
                    {col.header}
                  </th>
                ))}
              </tr>
            </thead>
            <tbody>
              {list.map((provider) => (
                <tr
                  key={provider.id}
                  onClick={() => handleRowClick(provider)}
                  className="border-b last:border-b-0 hover:bg-muted/50 cursor-pointer transition-colors"
                >
                  {providerColumns.map((col) => (
                    <td key={col.accessorKey || col.id} className="px-4 py-3">
                      {col.cell
                        ? col.cell({ row: { getValue: (key) => provider[key], original: provider } })
                        : provider[col.accessorKey]}
                    </td>
                  ))}
                </tr>
              ))}
            </tbody>
          </table>
        </div>
      )}
    </div>
  );
}

export default ProviderList;
