import { useState } from 'react';
import { useNavigate } from 'react-router-dom';
import { useTags } from '../../../hooks/use-tags';
import { tagColumns } from './tag-columns';
import { TagCard } from './tag-card';
import { PageHeader } from '../../layout/PageHeader';
import { EmptyState } from '../../feedback/EmptyState';
import { LoadingPage } from '../../feedback/LoadingSpinner';
import { ErrorState } from '../../feedback/ErrorState';
import { Button } from '../../ui/button';
import { Input } from '../../ui/input';
import { Badge } from '../../common/Badge';
import { Tag, Plus, Search, LayoutGrid, List } from 'lucide-react';

export function TagList({ onCreateClick }) {
  const navigate = useNavigate();
  const [search, setSearch] = useState('');
  const [viewMode, setViewMode] = useState('list');

  const filters = {};
  if (search) filters.search = search;

  const { data: tags, isLoading, error, refetch } = useTags(filters);

  const handleRowClick = (tag) => {
    navigate(`/tags/${tag.id}`);
  };

  if (isLoading) return <LoadingPage label="Loading tags..." />;
  if (error) return <ErrorState error={error} onRetry={refetch} />;

  const list = tags || [];

  return (
    <div className="flex flex-col gap-6">
      <PageHeader
        title="Tags"
        description="Manage your tags"
        icon={Tag}
        actions={
          <Button onClick={onCreateClick}>
            <Plus className="mr-2 h-4 w-4" />
            Add Tag
          </Button>
        }
      />

      <div className="flex items-center gap-3">
        <div className="relative flex-1 max-w-sm">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 size-4 text-muted-foreground" />
          <Input
            placeholder="Search tags..."
            value={search}
            onChange={(e) => setSearch(e.target.value)}
            className="pl-9"
          />
        </div>
        <div className="flex items-center gap-1 border rounded-md p-0.5">
          <Button
            variant={viewMode === 'list' ? 'secondary' : 'ghost'}
            size="icon"
            className="size-8"
            onClick={() => setViewMode('list')}
          >
            <List className="size-4" />
          </Button>
          <Button
            variant={viewMode === 'grid' ? 'secondary' : 'ghost'}
            size="icon"
            className="size-8"
            onClick={() => setViewMode('grid')}
          >
            <LayoutGrid className="size-4" />
          </Button>
        </div>
        <Badge variant="secondary">{list.length}</Badge>
      </div>

      {list.length === 0 ? (
        <EmptyState
          icon={Tag}
          title="No tags found"
          description={search ? 'Try a different search term.' : 'Add your first tag to get started.'}
          action={
            !search && (
              <Button onClick={onCreateClick}>
                <Plus className="mr-2 h-4 w-4" />
                Add Tag
              </Button>
            )
          }
        />
      ) : viewMode === 'grid' ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {list.map((tag) => (
            <TagCard
              key={tag.id}
              tag={tag}
              onClick={handleRowClick}
            />
          ))}
        </div>
      ) : (
        <div className="rounded-lg border">
          <table className="w-full">
            <thead>
              <tr className="border-b bg-muted/50">
                {tagColumns.map((col) => (
                  <th
                    key={col.accessorKey || col.id}
                    className="h-10 px-4 text-left text-xs font-medium text-muted-foreground"
                  >
                    {col.header}
                  </th>
                ))}
              </tr>
            </thead>
            <tbody>
              {list.map((tag) => (
                <tr
                  key={tag.id}
                  onClick={() => handleRowClick(tag)}
                  className="border-b last:border-b-0 hover:bg-muted/50 cursor-pointer transition-colors"
                >
                  {tagColumns.map((col) => (
                    <td key={col.accessorKey || col.id} className="px-4 py-3">
                      {col.cell
                        ? col.cell({ row: { getValue: (key) => tag[key], original: tag } })
                        : tag[col.accessorKey]}
                    </td>
                  ))}
                </tr>
              ))}
            </tbody>
          </table>
        </div>
      )}
    </div>
  );
}

export default TagList;
